 ...
 *
 * @param $properties
 *   If $validate_custom then the merged array of properties, otherwise only
 *   the properties for the group.
 * @param $errors
 *   If $validate_custom then the list of errors generated by conduit during
 *   validation of the base properties, otherwise an empty array.
 * @param $validate_custom
 *   If TRUE then custom validation of base properties is permitted, otherwise
 *   only validation of non-base properties is permitted.
 * @return
 *   If $validate_custom then the returned list will be the complete list of
 *   errors, otherwise the list will be appended to list of errors generated by
 *   other modules.
 */
function hook_conduit_conduit_validate(array $properties, array $errors, $validate_custom) {
  // Validate properties.
  foreach ($properties as $key => $value) {
    switch ($key) {
      case 'custom':
        if (!is_numeric($value)) {
          $errors[] = t('The <em>custom</em> property must be a numberic.');
        }
        break;
      case 'database':
        if ($validate_custom) {
          conduit_validate_error_remove($errors, 'database');
          if ($value != 'mysql') {
            $errors[] = t('The <em>database</em> property must be set to "mysql".');
          }
        }
        break;
    }
